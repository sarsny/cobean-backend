...
async function createThought() {
  try {
    ...
    const thoughtData = {
      title: "Test Thought for Chat",
      content: "This is a test thought to create a chat session",
      description: "A test thought description for chat testing",
    ...
    console.log('创建 Thought 成功:', response.status);
    console.log('Thought 数据:', JSON.stringify(response.data, null, 2));
    
    return response.data.data;
  } catch (error) {
    console.error('创建 Thought 失败:');
    if (error.response) {
...
async function createChat(thoughtId, beanId) {
  try {
    console.log('\n=== 创建新的聊天会话 ===');
...
async function main() {
  try {
    ...
    const beanId = thought.bean_id;
    
    // 2. 创建聊天会话
    const chat = await createChat(thoughtId, beanId);
    const conversationId = chat.id;
    
    // 3. 发送消息 (测试 POST /api/v1/chats/{conversationId}/messages)
    await sendMessage(conversationId);
    
    console.log('\n✅ 所有测试完成');
    console.log(`测试的 Thought ID: ${thoughtId}`);
